...
class OauthLogin {
	...
	public function userDetails($user_session) 
	{
		...
		$query = mysql_query("SELECT * FROM `users` WHERE uid = '$row_id'") or die(mysql_error());
	...
	$query = mysql_query("SELECT uid,provider FROM `users` WHERE email = '$email'") or die(mysql_error());
...
mysql_query("INSERT INTO users (email, name, first_name, last_name, gender, birthday, location, hometown, bio, relationship, timezone, provider, provider_id) VALUES ('$email','$name','$first_name','$last_name','$gender','$birthday','$location','$hometown','$bio','$relationship','$timezone','$loginProvider','$provider_id')");	
       
		$success_query = mysql_query("SELECT uid FROM `users` WHERE email = '$email'") or die(mysql_error());
